 ...
 *
 * @author Kurt Mammen
 * @version 04/12/12 - Added tests for BasicCollections (sort).
...
public class P2TestDriver
{
   ...
   private static boolean testBasicArrayListArch() throws ClassNotFoundException
   {
      ...
      //TODO: ADD STUFF TO CHECK FOR!
      ArrayList<String> list = new ArrayList<String>();
      list.add("java.util.AbstractList");
      list.add(" AbstractList");
      list.add("java.util.AbstractSequentialList");
      list.add(" AbstractSequentialList");
      list.add("java.util.ArrayList");
      list.add(" ArrayList");
      list.add("java.util.LinkedList");
      list.add(" LinkedList");
      list.add("java.util.Stack");
      list.add(" Stack");
      list.add("java.util.Vector");
      list.add(" Vector");

      pass &= testSource(list, "BasicArrayList.java");
      pass &= testSuppressWarnings(3, "BasicArrayList.java");
   ...
   private static boolean testBasicCollectionsArch() throws ClassNotFoundException
   {
      boolean pass = true;
      int cnt;
      Class cl;
      ...
      System.out.println("BasicCollections arcitecture tests...");

      cl = BasicCollections.class;

      ArrayList<String> list = new ArrayList<String>();
      list.add("java.util.Collections");
      list.add("java.util.Arrays");
      list.add("Arrays.sort");
      list.add("Collections.sort");

      pass &= testSource(list, "BasicCollections.java");
      pass &= testSuppressWarnings(0, "BasicCollections.java");
      
      pass &= test(cl.getSuperclass() == Class.forName("java.lang.Object"),
              "Class extends something other than Object");

      temp = cl.getInterfaces();
      pass &= test(temp.length == 0,
              "Incorrect number of interfaces, expected 0, found " + temp.length);

      pass &= test(cl.getConstructors().length == 1, "Incorrect number of constructors");

      cnt = countModifiers(cl.getDeclaredMethods(), Modifier.PUBLIC);     
      pass &= test(cnt == 0, "Non-static public methods found, none expected");
                 
      cnt = cl.getFields().length;
      pass &= test(cnt == 0, cnt + " public fields declared, 0 expected");
      
      cnt = countModifiers(cl.getDeclaredFields(), Modifier.PROTECTED);
      pass &= test(cnt == 0, "protected instance fields declared, none expected");
      
      cnt = countModifiers(cl.getDeclaredFields(), Modifier.PRIVATE);
      pass &= test(cnt == 0, cnt + " private instance fields declared, 0 expected");
   ...
   private static boolean testAddAtIndex()
   {
      ...
      BasicArrayList<Double> list = new BasicArrayList<Double>();
   ...
   private static boolean testClear()
   {
      ...
      BasicArrayList<Point> list = new BasicArrayList<Point>();
      list.clear();
      expected = new Point[] {};
      pass &= testList(list, expected, 0, BasicArrayList.DEFAULT_CAPACITY);
   ...
   private static boolean testSet()
   {
      ...
      BasicArrayList<String> list = new BasicArrayList<String>();
   ...
   /* Note: I am passing a variety of different BasicArrayList objects in.
    *       Since I am treating them as the raw type I need to suppress
    *       warnings.
    */
   ...
   private static boolean testBasicCollections()
   {
      System.out.println("\nTesting BasicCollections.sort(BasicList)...");
      boolean pass = true;

      // A local nested Comparator class needed for testing...
      ...
      // Make a semi-large BasicArrayList of puedo-random int values
      int size = 500;

      // Seed the random number generator to make debugging easier
      Random random = new Random(987);

      BasicArrayList<Integer> list = new BasicArrayList<Integer>(size);
      ...
      // Call the sort...
      BasicCollections.sort(list, new Comp());

      // Check the resultsi are in order...
      int dups = 0;

      for (int i = 1; i < size; i++)
      {
         int order = list.get(i-1).compareTo(list.get(i));
         
         if (order > 0)
         {
            pass &= test(false, "list is out of order after call to sort");
            break;
         }
         else if (order == 0)
         {
      ...
      pass &= test(dups == 0, "Data appears to have been corrupted by sort");
   ...
   private static boolean testSource(ArrayList<String> list, String fname)
   {
      boolean pass = true;
      Scanner scanner;
      ...
      {
         test(false, "You must run the test driver in them same directory as "
              + fname);
         return false;
      }
      ...
      while (scanner.hasNextLine())
      {
         ...
         // Make sure the line does not contain any string in the list...
         for(String s : list)
         {
            pass &= test(!line.contains(s), "You are not allowed to use "
                                          + s + " in your solution");
         }
      } 
   ...
   private static boolean testSuppressWarnings(int expected, String fname)
   {
      boolean pass = true;
      Scanner scanner;
      int countSuppressWarnings = 0;
      ...
      while (scanner.hasNextLine())
      {
         ...
         {
            countSuppressWarnings++;
         }
      }

      pass &= test(countSuppressWarnings <= expected,
         "Expected " + expected + " or fewer uses of @SupressWarnings, found "
         + countSuppressWarnings);
