...
void loadParticles(const char *filename)
{
	...
	while(getline(in, line))
    {
        ...
        double mass = stod(strtok(temp, " "));
        
        p->setMass(mass);
        p->setPosition(Vector3d(stod(strtok(NULL, " ")), stod(strtok(NULL, " ")), stod(strtok(NULL, " "))));
        p->setVelocity(Vector3d(stod(strtok(NULL, " ")), stod(strtok(NULL, " ")), stod(strtok(NULL, " "))));
        p->setColor(Vector3f(stof(strtok(NULL, " ")), stof(strtok(NULL, " ")), stof(strtok(NULL, " "))));
        p->setRadius(stof(strtok(NULL, " ")));
...
void createParticles()
{
	...
	double massHeavy = 1e-3;
    double r = 1.0;
    double a = 1.0;
    double y = sqrt(massHeavy * ((2.0 / r) - (1.0 / a)));
    
    shared_ptr<Particle> heavy = make_shared<Particle>();
    heavy->setMass(massHeavy);
    heavy->setPosition(Vector3d::Zero());
    heavy->setVelocity(Vector3d::Zero());
    
    shared_ptr<Particle> light = make_shared<Particle>();
    light->setMass(1e-6);
...
{
    vector<Vector3d> forces; 
    forces.resize(particles.size(), Vector3d::Zero());
    
    // Calculates the forces for all particles
    #pragma omp parallel for
    for (int i = 0; i < particles.size(); i++)
    {
        for (int j = i + 1; j < particles.size(); j++)
        {
            shared_ptr<Particle> p1 = particles[i];
            shared_ptr<Particle> p2 = particles[j];
            
            double massMult = p1->getMass() * p2->getMass();
            Vector3d displacement = p2->getPosition() - p1->getPosition();
            double displamentNorm = displacement.norm();
            
            double calculation = massMult / sqrt(pow(pow(displamentNorm, 2) + e2, 3));
            
            forces[i] += calculation * displacement;
            forces[j] += calculation * -displacement;
        }       
    }
    ...
    for (int i = 0; i < particles.size(); i++)
    {
        ...
        p->setVelocity(p->getVelocity() + h * forces[i] / p->getMass());
        p->setPosition(p->getPosition() + h * p->getVelocity());
    }
}
...
int main(int argc, char **argv)
{
	...
	if(sscanf(argv[1], "%i", &steps)) {
        ...
        //#pragma offload target(mic) inout(particles)
			stepParticles();
		}
	} else {
      ...
      glfwSetTime(0.0f);
		while(!glfwWindowShouldClose(window)) {
         ...
         float fpsTime = currTime - startTime;
         framesDrawn++;

         if(fpsTime >= 1.0f) {
            cout << "FPS: " << framesDrawn / fpsTime << endl;
            framesDrawn = 0;
            startTime = currTime;
         }
   }
