...
void cudaInitParticles(XYZ *locs, XYZ *vels, double *mass, int numParts, double h, double e2);
}
#endif

extern "C"; 
extern void cudaStepParticles(XYZ *locs);

extern void cudaCleanup();

extern void cudaInitParticles(XYZ *locs, XYZ *vels, double *mass, int numParts, double h, double e2);
...
public:
	bool operator()(size_t i0, size_t i1) const
	{
		...
		float z0 = V.row(2) * Vector4f(x0.x, x0.y, x0.z, 1.0f);
		float z1 = V.row(2) * Vector4f(x1.x, x1.y, x1.z, 1.0f);
		return z0 < z1;
	}
...
void renderGL()
{
	...
	for(int i = 0; i < sortedIndices.size(); ++i) {
		...
		particles[ii]->draw(locs[ii], prog, MV);
	}
	texture->unbind(0);
	prog->unbind();
...
void loadParticles(const char *filename)
{
  ...
  pCount = n;
  locs = (XYZ *)malloc(sizeof(XYZ) * n);
  vels = (XYZ *)malloc(sizeof(XYZ) * n);
  ...
  int id = 0;
	...
	while(getline(in, line))
    {
        ...
        p->setId(id);
        mass[id] = stod(strtok(temp, " "));

        XYZ *l = locs + id;
        XYZ *v = vels + id;
        
        l->x = stod(strtok(NULL, " "));
        l->y = stod(strtok(NULL, " "));
        l->z = stod(strtok(NULL, " "));
        
        v->x = stod(strtok(NULL, " "));
        v->y = stod(strtok(NULL, " "));
        v->z = stod(strtok(NULL, " "));
...
int main(int argc, char **argv)
{
  ...
  cudaInitParticles(locs, vels, mass, pCount, h, e2);
	// Try parsing `steps`
	int steps;
	if(sscanf(argv[1], "%i", &steps)) {
      ...
      while(!glfwWindowShouldClose(window)) {
         ...
         if (fpsTime >= 1.0f) {
            ...
            //cout << "Not enough time " << fpsTime << endl;
         }
		}
		// Quit program.
