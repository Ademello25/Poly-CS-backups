...
public class BST<E extends Comparable<? super E>> implements Iterable<E>
{
   ...
   private class Iterate implements Iterator<E>
   {
      ...
      SimpleArrayStack<BSTNode> stack;
      BSTNode current;


     public Iterate(BSTNode root)
      {
         stack = new SimpleArrayStack<BSTNode>();
         current = root;
         
         while(current.getClass() != EmptyNode.class)
         {
            stack.push(current);
      ...
      {
         return !(stack.size() == 0) || (current.getClass()!=EmptyNode.class);
      }
      

      public E next()
      {
         while((stack.size()>0) || (current.getClass() != EmptyNode.class))
         {
            if(current.getClass() != EmptyNode.class)
            {
               stack.push(current);
               current = ((Node)current).left;
            }
