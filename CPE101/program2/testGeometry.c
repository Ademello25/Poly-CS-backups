...
static Rectangle eir[] = {{{-2, -2}, { 2,  2}}, /* Primary rectangle */
                          {{-2, -2}, { 2,  2}}, /* identical */
                          {{-1, -1}, { 1,  1}}, /* smaller and completely bounded */
                          {{ 1,  1}, { 2,  2}}, /* intersects upper right corner */
                          {{ 1, -2}, { 2, -1}}, /* intersects lower right corner */
                          {{-2, -2}, {-1, -1}}, /* intersects lower left corner */
                          {{-2,  1}, {-1,  2}}, /* intersects upper left corner */
                          {{-2, -1}, { 2,  1}}, /* intersects horizontally and through */
                          {{-1, -2}, { 1,  2}}, /* intersects veritcally and through */
                          {{-1,  1}, { 1,  2}}, /* intersects vertically and up */
                          {{ 1, -1}, { 2,  1}}, /* intersects horizontall and right */
                          {{-1, -2}, { 1, -1}}, /* intersects veritally and down */
                          {{-2, -1}, {-1,  1}}, /* intersects horizontally and left */
                          {{-2, -2}, {-1,  2}}, /* intersects left side and through */
                          {{ 1, -2}, { 2,  2}}, /* intersects right side and through */
                          {{-2,  1}, { 2,  2}}, /* intersects top side and through */
                          {{-2, -2}, { 2, -1}}, /* intersects bottom side and through */
                          {{ 0,  0}, { 0,  0}}  /* TERMINATOR */
                         };
...
static Rectangle ebir[] = {{{-2, -2}, { 2,  2}}, /* Primary rectangle */
                           {{-2, -2}, { 2,  2}}, /* identical */
                           {{-2, -2}, { 2,  2}}, /* smaller and completely bounded */
                           {{-2, -2}, { 3,  3}}, /* intersects upper right corner */
                           {{-2, -3}, { 3,  2}}, /* intersects lower right corner */
                           {{-3, -3}, { 2,  2}}, /* intersects lower left corner */
                           {{-3, -2}, { 2,  3}}, /* intersects upper left corner */
                           {{-3, -2}, { 3,  2}}, /* intersects horizontally and through */
                           {{-2, -3}, { 2,  3}}, /* intersects veritcally and through */
                           {{-2, -2}, { 2,  3}}, /* intersects vertically and up */
                           {{-2, -2}, { 3,  2}}, /* intersects horizontall and right */
                           ...
                           {{-3, -3}, { 3,  2}}, /* intersects bottom side and through */
                           {{ 0,  0}, { 0,  0}}  /* TERMINATOR */
                          };

/* Expected bounding rectangles for non-intersecting rectangles */
static Rectangle ebnir[] = {{{-2, -2}, { 2,  2}}, /* Primary rectangle */
                            {{-2, -2}, { 4,  4}}, /* Touching upper right corner */
                            {{-2, -2}, { 4,  2}}, /* Touching right side */
                            {{-2, -4}, { 4,  2}}, /* Touching lower right corner */
                            {{-2, -4}, { 2,  2}}, /* Touching bottom side */
                            ...
                            {{-4, -2}, { 2,  4}}, /* Touching upper left corner */
                            {{-2, -2}, { 2,  4}}, /* Touching top side */
                            {{-2, -4}, { 6,  2}}, /* Outside lower right */
                            {{-2, -6}, { 2,  2}}, /* Outside bottom */
                            {{-6, -6}, { 2,  2}}, /* Outside lower left */
                            {{-6, -2}, { 2,  2}}, /* Outside left */
                            {{-6, -2}, { 2,  6}}, /* Outside upper left */
                            {{-2, -2}, { 2,  6}}, /* Outside top */
                            {{-2, -2}, { 6,  6}}, /* Outside upper right */
                            {{-2, -2}, { 6,  2}}, /* Outside right */
                            {{ 0,  0}, { 0,  0}}  /* TERMINATOR */
                           };
...
static int testMakeRectangle()
{
   ...
   pass &= testInt(-10, r.ll.x, __LINE__);
   pass &= testInt(-20, r.ll.y, __LINE__);

   pass &= testInt(-8, r.ur.x, __LINE__);
   pass &= testInt(-16, r.ur.y, __LINE__);
...
static int testWidth()
{
   ...
   Rectangle r = makeRectangle (-10, -20, 2, 4);
   pass &= testInt(2, width(r), __LINE__);

   pass &= testFalse(valid(invalidRect), __LINE__);
   pass &= testInt(0, width(invalidRect), __LINE__);
...
static int testHeight()
{
   ...
   pass &= testFalse(valid(invalidRect), __LINE__);
   pass &= testInt(0, height(invalidRect), __LINE__);
...
static int testArea()
{
   ...
   Rectangle r = makeRectangle (-10, -20, 12, 24);
   pass &= testInt(288, area(r), __LINE__);
   
   pass &= testFalse(valid(invalidRect), __LINE__);
   pass &= testInt(0, area(invalidRect), __LINE__);
...
static int testPerimeter()
{
   ...
   Rectangle r = makeRectangle (-10, -20, 2, 5);
   pass &= testInt(14, perimeter(r), __LINE__);

   pass &= testFalse(valid(invalidRect), __LINE__);
   pass &= testInt(0, perimeter(invalidRect), __LINE__);
...
static int testValid()
{
   ...
   Rectangle r = makeRectangle (-10, -20, 2, 5);
   pass &= testTrue(valid(r), __LINE__);
...
static int testIntersects()
{
   ...
   while ( (ir[i].ll.x || ir[i].ll.y || ir[i].ur.x || ir[i].ur.y) )
   {
      /* Make sure rectangles have not been corrupted by student's code */
      /* NOTE: Not a perfect test as they could be corrupt but still valid rectangle! */
      pass &= testTrue( valid(ir[0]), __LINE__ );
      pass &= testTrue( valid(ir[i]), __LINE__ );

      /* Test for intersection */
      pass &= testTrue( intersects(ir[0], ir[i]), __LINE__ );
      pass &= testTrue( intersects(ir[i], ir[0]), __LINE__ );

      /* Test with invalid rects - should return false if either or both are not valid */
      pass &= testFalse(valid(invalidRect), __LINE__);
      pass &= testFalse( intersects(ir[0], invalidRect), __LINE__ );
   ...
   /* Test all non-intersections */
   i = 1;

   while ( (nir[i].ll.x || nir[i].ll.y || nir[i].ur.x || nir[i].ur.y) )
   {
      /* Make sure rectangles have not been corrupted by student's code */
      /* NOTE: Not a perfect test as they could be corrupt but still valid rectangle! */
      pass &= testTrue( valid(nir[0]), __LINE__ );
      pass &= testTrue( valid(nir[i]), __LINE__ );

      /* Test for lack of intersection */
      pass &= testTrue( valid(nir[i]), __LINE__ );
      pass &= testFalse( intersects(nir[0], nir[i]), __LINE__ );
      pass &= testFalse( intersects(nir[i], nir[0]), __LINE__ );
      i++;
   }
...
static int testBoundingRectangle()
{
   ...
   /* Test bounding rectangles for all intersecting rectangles */
   i = 1;
   ...
   while ( (ebir[i].ll.x || ebir[i].ll.y || ebir[i].ur.x || ebir[i].ur.y) )
   {
      ...
      pass &= testTrue( valid(ir[0]), __LINE__ );
      pass &= testTrue( valid(ir[i]), __LINE__ );
      pass &= testTrue( valid(ebir[i]), __LINE__ );

      /* Test bounding rectangles */
      pass &= testRect(ebir[i], boundingRectangle(ir[0], ir[i]), __LINE__);
      pass &= testRect(ebir[i], boundingRectangle(ir[i], ir[0]), __LINE__);
      i++;
   }

   /* Test bounding rectangles for all non-intersecting rectangles */
   i = 1;

   while ( (ebnir[i].ll.x || ebnir[i].ll.y || ebnir[i].ur.x || ebnir[i].ur.y) )
   {
      /* Make sure rectangles have not been corrupted by student's code */
      /* NOTE: Not a perfect test as they could be corrupt but still valid rectangle! */
      pass &= testTrue( valid(nir[0]), __LINE__ );
      ...
      pass &= testRect(ebnir[i], boundingRectangle(nir[i], nir[0]), __LINE__);
      i++;
   }
...
static int testIntersection()
{
   int pass = 1;
   int i;
   Rectangle enir = {{0, 0}, {0, 0}};
   
   /* Test rectangles of intersection for all intersecting rectangles */
   i = 1;

   while ( (eir[i].ll.x || eir[i].ll.y || eir[i].ur.x || eir[i].ur.y) )
   {
      /* Make sure rectangles have not been corrupted by student's code */
      /* NOTE: Not a perfect test as they could be corrupt but still valid rectangle! */
      pass &= testTrue( valid(ir[0]), __LINE__ );
      pass &= testTrue( valid(ir[i]), __LINE__ );
      pass &= testTrue( valid(eir[i]), __LINE__ );

      /* Test bounding rectangles */
      pass &= testRect(eir[i], intersection(eir[0], eir[i]), __LINE__);
   ...
   while ( (nir[i].ll.x || nir[i].ll.y || nir[i].ur.x || nir[i].ur.y) )
   {
      ...
      pass &= testTrue( valid(nir[0]), __LINE__ );
      pass &= testTrue( valid(nir[i]), __LINE__ );
      pass &= testFalse( valid(enir), __LINE__ );

      /* Test bounding rectangles */
      pass &= testRect(enir, intersection(nir[0], nir[i]), __LINE__);
      pass &= testRect(enir, intersection(nir[i], nir[0]), __LINE__);
      i++;
   }
...
static void results(int pass)
{
   if (pass)
   {
      ...
      printf("assignment, your grade will be based on when you hand it in, minus\n");
...
int testTrue(int value, int line)
{
   ...
   {
      fprintf(stderr,
              "   Failed test at line %d, expected true, found false\n", line);

      return 0;
   }

   /* Must return 1, NOT value, because not all true values (non-zero)
    * have low-order bit set and &= operator uses the low-order bit!
    */
   return 1;
}
...
int testFalse(int value, int line)
{
   ...
   {
      fprintf(stderr,
              "   Failed test at line %d, expected false, found true\n", line);
...
int testRect(Rectangle expect, Rectangle actual, int line)
{
   if (expect.ll.x != actual.ll.x
    || expect.ll.y != actual.ll.y
    || expect.ur.x != actual.ur.x
    || expect.ur.y != actual.ur.y)
   {
      fprintf(stderr,
              "   Failed test at line %d, expected {{%d, %d}, {%d, %d}}, ",
              line, expect.ll.x, expect.ll.y, expect.ur.x, expect.ur.y);

      fprintf(stderr,
              "found {{%d, %d}, {%d, %d}}\n",
              actual.ll.x, actual.ll.y, actual.ur.x, actual.ur.y);
