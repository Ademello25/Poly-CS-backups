...
int classify_type(const ALL_Types data) {
  ...
  if (data.type.alu.instr.class_type.type_check == ALU_TYPE) {
    /*if (data.type.alu.instr.lsli.op == ALU_LSLI_OP) {
       ...
       cout << "adds r" << 
        setbase(10) << (static_cast<unsigned int>(data.type.alu.instr.addr.rt));
       cout << ", r" <<
        setbase(10) << (static_cast<unsigned int>(data.type.alu.instr.addr.rn));
       cout << ", r" <<
        setbase(10) << (static_cast<unsigned int>(data.type.alu.instr.addr.rm)) << endl;
    }
    /*else if (data.type.alu.instr.subr.op == ALU_SUBR_OP) {
    }*/
    else if (data.type.alu.instr.add3i.op == ALU_ADD3I_OP) {
      cout << "adds r" <<
       setbase(10) << (static_cast<unsigned int>(data.type.alu.instr.add3i.rt));
      cout << ", r" <<
       setbase(10) << (static_cast<unsigned int>(data.type.alu.instr.add3i.rn));
      cout << ", #" << 
       setbase(10) << (static_cast<unsigned int>(data.type.alu.instr.add3i.imm)) << endl;
    }
    /*else if (data.type.alu.instr.sub3i.op == ALU_SUB3I_OP) {
    }*/
    else if (data.type.alu.instr.add8i.op == ALU_ADD8I_OP) {
      cout << "adds r" <<
       setbase(10) << (static_cast<unsigned int>(data.type.alu.instr.add8i.rt));
      cout << ", #" <<
       setbase(10) << (static_cast<unsigned int>(data.type.alu.instr.add8i.imm)) << endl;
    }
    /*else if (data.type.alu.instr.sub8i.op == ALU_SUB8I_OP) {
    }*/
    else if (data.type.alu.instr.cmp.op == ALU_CMP_OP) {
      cout << "cmp r" <<
       setbase(10) << (static_cast<unsigned int>(data.type.alu.instr.cmp.rt));
      cout << ", #" <<
       setbase(10) << (static_cast<unsigned int>(data.type.alu.instr.cmp.imm)) << endl;
    }
    else if (data.type.alu.instr.mov.op == ALU_MOV_OP) {
      cout << "movs " << "r" << data.type.alu.instr.mov.rt;
    ...
    cout << "mov sp, r" << 
     setbase(10) << (static_cast<unsigned int>(data.type.sp.instr.mov.rm)) << endl;
    return SP_TYPE;
  }
  else if (data.type.uncond.instr.class_type.type_check == UNCOND_TYPE) {
    cout << "b 0x" << data.type.uncond.instr.b.imm << endl;
    return UNCOND_TYPE;
  }
  else 
  ...
  // Complete the rest of these
    ...
    cout << " 0x" << data.type.cond.instr.b.imm << endl;
    return COND_TYPE;
  }
...
//    return STM_TYPE;
    ...
    cout << "add r";
    cout << setbase(10) <<
     (static_cast<unsigned int>(data.type.addsp.instr.add.rt));
    cout << ", sp, #";
    cout << setbase(10) << 
     (static_cast<unsigned int>(data.type.addsp.instr.add.imm)) << endl;
    ...
    if (data.type.ld_st.instr.class_type.opA == LD_ST_REG_OPA) {
      ...
      if (data.type.ld_st.instr.class_type.opB == LD_ST_OPB_STR) {
         cout << "str r";
         cout << setbase(10) <<
          (static_cast<unsigned int>(data.type.ld_st.instr.ld_st_imm.rt));
         cout << ", [r";
         cout << setbase(10) <<
          (static_cast<unsigned int>(data.type.ld_st.instr.ld_st_imm.rn));
         cout << ", #";
         cout << setbase(10) <<
          (static_cast<unsigned int>(data.type.ld_st.instr.ld_st_imm.imm)<< 2);
         cout << "]" << endl;
      }
